...
// Directly interfaces with Supabase PostgreSQL via Drizzle ORM when DATABASE_URL is present,
// and gracefully falls back to structured JSON file storage in local testing environments.
...
// -----------------------------------------------------------------------------
// CRUD LAYER (Dynamic Drizzle/Postgres OR Mock-DB Switcher)
// -----------------------------------------------------------------------------
...
export async function getProducts(): Promise<Product[]> {
  if (pgDb) {
    ...
    const db = getMockDb();
    return db.products.filter((p: any) => p.active !== false);
  }
}
...
export async function createProduct(data: Omit<Product, 'id' | 'createdAt' | 'active'>): Promise<Product> {
  if (pgDb) {
    ...
    await logAudit('usr-superadmin', 'CREATE', 'PRODUCTS', `Created style template "${newProduct.productName}"`);
    return newProduct;
  }
}
...
export async function getProductColors(productId: string): Promise<ProductColor[]> {
  if (pgDb) {
    ...
    const db = getMockDb();
    return (db.colors || []).filter((c: any) => c.productId === productId);
  }
}
...
export async function getProductSizes(productId: string): Promise<ProductSize[]> {
  if (pgDb) {
    ...
    const db = getMockDb();
    return (db.sizes || []).filter((s: any) => s.productId === productId);
  }
}
...
export async function getVariants(productId?: string): Promise<ProductVariant[]> {
  if (pgDb) {
    ...
    const db = getMockDb();
    let list = db.variants || [];
    if (productId) {
      list = list.filter((v: any) => v.productId === productId);
    }
    return list.filter((v: any) => v.active !== false);
  }
}
...
export async function createVariant(data: Omit<ProductVariant, 'id' | 'createdAt' | 'active' | 'colorId' | 'sizeId'> & { colorName: string; sizeName: string }): Promise<ProductVariant> {
  if (pgDb) {
    ...
    // Enforce Barcode uniqueness
    const existingBarcode = (db.variants || []).find((v: any) => v.barcode && v.barcode.trim() === (data.barcode || '').trim());
    if (existingBarcode) {
      throw new Error(`Barcode error: Barcode "${data.barcode}" is already registered for SKU "${existingBarcode.sku}".`);
    }

    // Enforce SKU uniqueness
    const existingSku = (db.variants || []).find((v: any) => v.sku.toLowerCase() === data.sku.toLowerCase().trim());
    if (existingSku) {
      throw new Error(`SKU error: SKU "${data.sku}" is already registered.`);
    }

    // Find or create color
    let colorObj = (db.colors || []).find((c: any) => c.productId === data.productId && c.colorName.toLowerCase() === data.colorName.toLowerCase());
    if (!colorObj) {
      colorObj = {
    ...
    // Find or create size
    let sizeObj = (db.sizes || []).find((s: any) => s.productId === data.productId && s.sizeName.toLowerCase() === data.sizeName.toLowerCase());
    if (!sizeObj) {
      sizeObj = {
    ...
    const newVariant: ProductVariant = {
      ...
      productId: data.productId,
      sku: data.sku,
      barcode: data.barcode || '11000000000' + (db.variants.length + 1),
      colorId: colorObj.id,
      sizeId: sizeObj.id,
    ...
    await logAudit('usr-superadmin', 'CREATE', 'VARIANTS', `Registered new variant SKU "${newVariant.sku}"`);
    return newVariant;
  }
}
...
export async function createStockRequest(
  ...
  if (pgDb) {
    ...
    const db = getMockDb();
    const variant = db.variants.find((v: any) => v.id === data.variantId);
    if (!variant) return { success: false, error: 'SKU variant not found' };

    // Calculate live Ready Stock before processing dispatches
    const allTx = db.transactions || [];
    const currentStock = compileStockForVariant(data.variantId, allTx);

    // Ready Stock constraint check
    const qty = Number(data.quantity);
    const outTypes = ['SALE', 'DAMAGE_REPAIRABLE', 'DAMAGE_NON_REPAIRABLE', 'ADJUSTMENT_OUT'];
    if (outTypes.includes(data.requestType)) {
      if (currentStock.readyStock < qty) {
...
export async function reviewStockRequest(
  ...
  if (pgDb) {
    ...
    if (reqIdx === -1) return { success: false, error: 'Stock request not found.' };

    const req = db.requests[reqIdx];
    if (req.status !== 'PENDING') return { success: false, error: 'Request is already reviewed.' };

    const variant = db.variants.find((v: any) => v.id === req.variantId);
    if (!variant) return { success: false, error: 'SKU variant not found.' };

    // Double check Ready Stock balance before approving dispatches
    if (status === 'APPROVED') {
      const allTx = db.transactions || [];
      const currentStock = compileStockForVariant(req.variantId, allTx);
      const qty = Number(req.quantity);
      const outTypes = ['SALE', 'DAMAGE_REPAIRABLE', 'DAMAGE_NON_REPAIRABLE', 'ADJUSTMENT_OUT'];

      if (outTypes.includes(req.requestType) && currentStock.readyStock < qty) {
        return {
          success: false,
          error: `Action aborted! Stock deficit occurred: requested ${qty} units, but only ${currentStock.readyStock} are in stock.`
        };
      ...
      const newTx: StockTransaction = {
        ...
        requestId: req.id,
        variantId: req.variantId,
        transactionType: req.requestType,
        quantity: qty,
...
export async function getComputedInventory(): Promise<ComputedInventoryItem[]> {
  ...
  if (pgDb) {
    ...
    const db = getMockDb();
    const results: ComputedInventoryItem[] = [];
    const variantsList = db.variants || [];
    
    for (const v of variantsList) {
      if (v.active === false) continue;
      
      const prod = db.products.find((p: any) => p.id === v.productId);
      if (!prod || prod.active === false) continue;

      // Get Relational Names
      const colorObj = (db.colors || []).find((c: any) => c.id === v.colorId);
      const sizeObj = (db.sizes || []).find((s: any) => s.id === v.sizeId);
      ...
      if (stock.readyStock === 0) {
        status = 'Out of Stock';
      } else if (stock.readyStock <= 5) { // default low stock
        status = 'Low Stock';
      }
...
export async function getDashboardStats() {
  ...
  if (pgDb) {
    ...
    const totalProducts = db.products.filter((p: any) => p.active !== false).length;
    const totalVariants = inventory.length;
    let totalReady = 0;
    let totalRepairable = 0;
    let totalScrap = 0;
    ...
    const todayStr = new Date().toISOString().slice(0, 10);
    const approvedTodayCount = requests.filter((r: any) => r.status === 'APPROVED' && r.reviewedAt?.startsWith(todayStr)).length;
    const rejectedTodayCount = requests.filter((r: any) => r.status === 'REJECTED' && r.reviewedAt?.startsWith(todayStr)).length;
...
export async function getAuditLogs(): Promise<AuditLog[]> {
  if (pgDb) {
    ...
    const db = getMockDb();
    return [...(db.auditLogs || [])].sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  }
}
...
export async function getPriceHistory(variantId?: string): Promise<PriceHistory[]> {
  if (pgDb) {
    ...
    if (variantId) {
      list = list.filter((p: any) => p.variantId === variantId);
    }
...
export async function getTransactions(variantId?: string): Promise<StockTransaction[]> {
  if (pgDb) {
    ...
    if (variantId) {
      list = list.filter((t: any) => t.variantId === variantId);
    }
    return [...list].sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  }
}
